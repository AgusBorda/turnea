...
BEGIN
  ...
  SELECT s.duration, s.active
  INTO v_service_duration, v_service_active
  ...
  IF p_payment_pending THEN
    IF auth.role() IS DISTINCT FROM 'service_role'
      ...
      OR v_deposit_required IS NOT TRUE
      OR v_mp_configured IS NOT TRUE THEN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role'
    ...
    AND v_deposit_required
    AND v_mp_configured THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'PAYMENT_REQUIRED';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE block.barber_id = p_barber_id
      ...
      AND (
        block.all_day
        OR (block.start_time < v_end_time AND block.end_time > p_start_time)
      )
  ) THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'SLOT_UNAVAILABLE';
  END IF;
    ...
    WHERE appointment.barber_id = p_barber_id
      ...
      AND (
        ...
        OR (
          ...
          AND appointment.expires_at > v_now
        )
      )
  ) THEN
...
BEGIN
  ...
  IF v_appointment.status <> 'pending_payment'
    ...
    OR v_appointment.expires_at IS NULL
  ...
  IF EXISTS (
    ...
    WHERE other.id <> v_appointment.id
      ...
      AND (
        other.status IN ('pending', 'confirmed')
        OR (
          other.status = 'pending_payment'
          AND other.deposit_status = 'pending'
...
AS $function$
BEGIN
  IF auth.uid() IS NULL OR p_status NOT IN ('completed', 'cancelled', 'no_show') THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'NOT_AUTHORIZED';
  END IF;

  UPDATE public.appointments AS appointment
  SET status = p_status,
      cancelled_at = CASE WHEN p_status = 'cancelled' THEN clock_timestamp() ELSE appointment.cancelled_at END,
      cancelled_by = CASE WHEN p_status = 'cancelled' THEN 'owner' ELSE appointment.cancelled_by END
  FROM public.barbershops AS barbershop
  WHERE appointment.id = p_appointment_id
    AND barbershop.id = appointment.barbershop_id
...
BEGIN
  FOR v_policy_name IN
    ...
    WHERE schemaname = 'public'
      ...
      AND cmd = 'INSERT'
  LOOP
    EXECUTE format('DROP POLICY %I ON public.appointments', v_policy_name);
  END LOOP;
END;
$block$;

REVOKE ALL ON FUNCTION public.create_appointment_atomic(uuid, uuid, uuid, date, time without time zone, text, text, boolean, numeric, timestamp with time zone) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_appointment_atomic(uuid, uuid, uuid, date, time without time zone, text, text, boolean, numeric, timestamp with time zone) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.confirm_paid_appointment_atomic(uuid, uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.confirm_paid_appointment_atomic(uuid, uuid, text) TO service_role;

REVOKE ALL ON FUNCTION public.update_appointment_status_owner(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_appointment_status_owner(uuid, text) TO authenticated;
